    ...
    const preset = {    
        ...
        header: ({ props }) => ({
            class: [
                ...
                // Shape
                // props.showGridlines ? 'border-y border-x  border-b-0' : 'border-[1px]',
        ...
        table: {
            class: 'w-full border-spacing-0 border-surface-300 bg-[#f0f0f0] !h-full' // !h-auto перебивает inline style height
        },
        thead: ({ context }) => ({
            class: [
                {
                    'bg-[#B9B9B9] top-px z-40 sticky': context.scrollable
                }
            ]
        }),
        tbody: ({ instance, context }) => ({
            class: [
                'after:absolute',
                {
                    'sticky z-20 !h-full': instance.frozenRow && context.scrollable
                }
            ]
        }),
        tfoot: ({ context }) => ({
        ...
        column: {
            headercell: ({ context, props }) => ({
                class: [
                    ...
                    // Shape
                    'border border-solid last:border-r',
                    ...
                    // Color
                    (props.sortable === '' || props.sortable) && context.sorted ? 'bg-primary-50 text-primary-700' : 'bg-[#f0f0f0] text-slate-700',
                    ...
                    'bg-surface-200 border-[#B9B9B9]  dark:border-surface-700 bg-clip-padding',
            ...
            headercontent: {
                class: 'flex items-center min-h-[29px]'
            },
            sort: ({ context }) => ({
            ...
            bodycell: ({ props, context, state, parent }) => ({
                class: [
                    "min-w-[30px] max-w-[400px] text-sm text-slate-600 overflow-auto",
                    //Position
