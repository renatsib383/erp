const ptTabViewDealModalPreset = {
  root: {
    class: [
      'h-full',
      'overflow-hidden'
  ...
  panelcontainer: {
    class: [
      'h-full flex overflow-hidden gap-[1px]'
    ]
  },
  tabpanel: {
    header: ({ props }) => ({
    ...
    content: ({context}) => ({
      class: [
        'h-full basis-0 grow max-w-full lg:!block',
        // Spacing
        // context.index === 0 ? 'lg:max-w-[35%]' : 'lg:max-w-[65%]',
        context.index === 0 ? 'panel-left' : '',
...
export default ptTabViewDealModalPreset;
